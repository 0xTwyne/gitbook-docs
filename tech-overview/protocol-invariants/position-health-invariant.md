...
1. $$B \leq \tilde{\lambda}_t \cdot C$$ (Twyne condition)
2. $$B \leq \beta_{safe} \cdot \tilde{\lambda}_e \cdot (C+C_{LP})$$ (External protocol condition)
...
* $$\tilde{\lambda}_t$$ = Twyne liquidation LTV (user-selected parameter)
* $$\tilde{\lambda}_e$$ = External protocol liquidation LTV
* $$\beta_{safe}$$ = Safety buffer (0.95 to 1.0)
...
def check_position_health(user_collateral, reserved_credit, borrowed_amount,
                         twyne_liq_ltv, ext_lending_liq_ltv, safety_buffer=0.95,
    ...
    max_borrow_external = total_collateral * ext_lending_liq_ltv

    # Check both conditions
    twyne_condition_met = borrowed_amount <= max_borrow_twyne
    external_condition_met = borrowed_amount <= safety_buffer * max_borrow_external
...
result = check_position_health(
    ...
    twyne_liq_ltv, ext_lending_liq_ltv, safety_buffer
)
...
2. **Relationship to Rebalancing**: Rebalancing helps maintain both invariants by releasing excess credit and improving position health
